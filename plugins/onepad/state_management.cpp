		...
		case CMD_READ_DATA_AND_VIBRATE:
			{
				...
				if (pad->mode != MODE_DIGITAL) {
					...
					if (pad->mode != MODE_ANALOG) {
						query.response[9]  = !test_bit(buttons, 9)  ? key_status->get(query.port, PAD_RIGHT) : 0;
						query.response[10] = !test_bit(buttons, 11) ? key_status->get(query.port, PAD_LEFT) : 0;
						query.response[11] = !test_bit(buttons, 8)  ? key_status->get(query.port, PAD_UP) : 0;
						query.response[12] = !test_bit(buttons, 10) ? key_status->get(query.port, PAD_DOWN) : 0;
